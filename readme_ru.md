...
- **html_element(url:str, find_all_args, number:int=0)->str:** — получить текст HTML-элемента по указанной ссылке.
	*find_all_args* — словарь, который содержит информацию о нужном элементе (его имя, атрибуты)
...
## Примеры задач
	...
	def iPython(submenu='Rare', task_name='iPython + плагины'):
